...
PathToCDLRasters <- "E:\\FireflyAnalysis_October2020\\CDL_individual_pseudostates\\2011"
PseudostateNames <- data.frame(list.files(PathToCDLRasters, pattern = "\\.tif$")) # read all the files for 2011
names(PseudostateNames) <- c("ID") # add column header

PseudostateNames[] <- lapply(PseudostateNames, gsub, pattern = "CDL_2011_", replacement = "", fixed = TRUE) # tidy the names
...
for(PseudostateID in PseudostateNamesList){
  ...
  Focal_cdlpath <- paste0("E:\\FireflyAnalysis_October2020\\CDL_individual_pseudostates\\2011\\CDL_2011_", PseudostateID, ".tif")
  
  # define the path where the new pesticide map will go after SpeedyBeeModel is done
  FocalOutputDIr <- paste0("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2011\\", PseudostateID, "_SpeedyBee_", FocalYear)
  ...
  # SpeedyBeeOutput <- raster(paste0(FocalOutputDIr, "\\CDL_2011_East Texas_insecticide.tif"))
...
list.files("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2011") # seems like it worked??

# plot one to see how it looks
NorthMN2011map <- raster("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2011\\North Minnesota_SpeedyBee_2011\\CDL_2011_North Minnesota_insecticide.tif")
plot(NorthMN2011map) # looks good!
