...
PathToCDLRasters <- "E:\\FireflyAnalysis_October2020\\CDL_individual_pseudostates\\2014"
PseudostateNames <- data.frame(list.files(PathToCDLRasters, pattern = "\\.tif$")) # read all the files for 2014
names(PseudostateNames) <- c("ID") # add column header

PseudostateNames[] <- lapply(PseudostateNames, gsub, pattern = "CDL_2014_", replacement = "", fixed = TRUE) # tidy the names
...
for(PseudostateID in PseudostateNamesList){
  ...
  Focal_cdlpath <- paste0("E:\\FireflyAnalysis_October2020\\CDL_individual_pseudostates\\2014\\CDL_2014_", PseudostateID, ".tif")
  
  # define the path where the new pesticide map will go after SpeedyBeeModel is done
  FocalOutputDIr <- paste0("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2014\\", PseudostateID, "_SpeedyBee_", FocalYear)
  ...
  # SpeedyBeeOutput <- raster(paste0(FocalOutputDIr, "\\CDL_2014_East Texas_insecticide.tif"))
...
list.files("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2014") # seems like it worked??

# plot one to see how it looks
NorthMN2014map <- raster("E:\\FireflyAnalysis_October2020\\individual_pesticide_maps\\2014\\North Minnesota_SpeedyBee_2014\\CDL_2014_North Minnesota_insecticide.tif")
plot(NorthMN2014map) # looks good!
